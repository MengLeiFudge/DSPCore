...
/// 用于角落选择项的下拉框控件。
/// </summary>
public class UiCornerComboBox : MonoBehaviour {
    ...
    public static void InitBaseObject() {
        ...
        go.name = "dspcore-corner-combo-box";
        go.SetActive(false);
    ...
    public static UiCornerComboBox CreateComboBox(float x, float y, RectTransform parent) {
        var gameObject = Instantiate(_baseObject);
        gameObject.name = "dspcore-corner-combo-box";
        gameObject.SetActive(true);
        var cb = gameObject.AddComponent<UiCornerComboBox>();
    ...
    public UiCornerComboBox WithFontSize(int size) {
        SetFontSize(size);
        return this;
    }

    public UiCornerComboBox WithItems(params string[] items) {
        SetItems(items);
        return this;
    }

    public UiCornerComboBox WithIndex(int index) {
        SetIndex(index);
        return this;
    }

    public UiCornerComboBox WithSize(float width, float height) {
        SetSize(width, height);
        return this;
    }

    public UiCornerComboBox WithOnSelChanged(params Action<int>[] action) {
        foreach (var act in action)
            AddOnSelChanged(act);
        return this;
    }

    public UiCornerComboBox WithConfigEntry(ConfigEntry<int> config) {
