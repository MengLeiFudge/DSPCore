...
public class UiImageButton : MonoBehaviour {
    ...
    //是否选中与按钮组
    private UiImageButtonGroup _buttonGroup = null;
    ...
    public static void InitBaseObject() {
        ...
        // 创建一个新的GameObject
        _baseObject = new("dspcore-image-button-base");
    ...
    public static UiImageButton CreateImageButton(float x, float y, RectTransform parent,
        Proto proto, float width = 40f, float height = 40f,
        string tipTitle = "", string tipContent = "") {
        var go = Instantiate(_baseObject);
        go.name = "dspcore-image-button";
        go.SetActive(true);

        var ibtn = go.AddComponent<UiImageButton>();
    ...
    public UiImageButton WithClickEvent(UnityAction onLeftClick, UnityAction onRightClick) {
        uiButton.onClick += _ => onLeftClick?.Invoke();
        uiButton.onRightClick += _ => onRightClick?.Invoke();
        return this;
    }

    public UiImageButton WithSize(float width, float height) {
        rectTrans.sizeDelta = new(width, height);
        return this;
    }

    public UiImageButton WithTip(string tip, float delay = 1f) {
    ...
    public void SetCount(long count) {
        SetCountText(FormatCompact(count));
    }
    ...
    public UiImageButton WithCount(long count) {
        SetCount(count);
        return this;
    }
    ...
    public UiImageButton WithSelected(bool selected) {
        Selected = selected;
        return this;
    }

    public UiImageButton WithGroup(UiImageButtonGroup group) {
        _buttonGroup = group;
        group?.AddButton(this);
        return this;
    }

    public UiImageButton WithDeselectOnHover(bool enable = true, UnityAction onDeselect = null) {
