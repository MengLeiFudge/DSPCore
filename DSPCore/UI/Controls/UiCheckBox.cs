...
// UiCheckBox modified from LSTM: https://github.com/hetima/DSP_LSTM/blob/main/LSTM/UiCheckBox.cs
/// <summary>
/// 勾选框
/// </summary>
public class UiCheckBox : MonoBehaviour {
    ...
    public static UiCheckBox CreateCheckBox(float x, float y, RectTransform parent, ConfigEntry<bool> config,
        string label = "", int fontSize = 15) {
        return CreateCheckBox(x, y, parent, config.Value, label, fontSize).WithConfigEntry(config);
    }

    public static UiCheckBox CreateCheckBox(float x, float y, RectTransform parent, bool check, string label = "",
        int fontSize = 15) {
        return CreateCheckBox(x, y, parent, fontSize).WithCheck(check).WithLabelText(label);
    }

    public static UiCheckBox CreateCheckBox(float x, float y, RectTransform parent, int fontSize = 15) {
        var go = Instantiate(_baseObject);
        go.name = "dspcore-check-box";
        go.SetActive(true);
        var cb = go.AddComponent<UiCheckBox>();
    ...
    public void SetEnable(bool on) {
        ...
        if (on) {
            ...
            if (boxImage) boxImage.color = MultiplyRgb(BoxColor, 0.5f);
            if (checkImage) checkImage.color = MultiplyRgb(CheckColor, 0.5f);
            if (labelText) labelText.color = MultiplyRgb(TextColor, 0.5f);
        }
    }
    ...
    public UiCheckBox WithLabelText(string val) {
        SetLabelText(val);
        return this;
    }

    public UiCheckBox WithCheck(bool check) {
        Checked = check;
        return this;
    }

    public UiCheckBox WithSmallerBox(float boxSize = 20f) {
    ...
    public UiCheckBox WithEnable(bool on) {
        SetEnable(on);
        return this;
    }

    public UiCheckBox WithConfigEntry(ConfigEntry<bool> config) {
