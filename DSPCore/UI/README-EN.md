...
It does not provide concrete business pages. A mod's own pages, navigation, unlock rules, save state, and business buttons remain owned by that mod or by the relevant feature block.
...
- Lightweight descriptor objects such as `UiButtonDescriptor` and `UiNodeFactory` for sharing UI metadata across feature blocks.
...
It only handles generic tabs inside a custom window. It is not the same as item, recipe, or replicator tabs in the game UI; those remain owned by the `Tabs` feature block.
...
- It does not replace feature blocks that already have clear ownership, such as `Tabs`, `Pickers`, and `Errors`.
