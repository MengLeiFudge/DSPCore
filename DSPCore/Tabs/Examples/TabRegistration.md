...
在启动阶段注册一次。当前投射覆盖原版物品选择器、配方选择器、制造器页面、信号选择器和标签图标选择器。蓝图图标、描述图标、智能输入框图标等复用原版 signal/tag picker 的界面会受益；检测到 GenesisBook、OrbitalRing 或 FE 接管 signal/tag picker 时会避免重复注入按钮，真正自建且不复用原版 picker 的第三方界面仍需要专门适配。
