...
如果错误发生时已经有明确的星球工厂或实体 ID，可以把上下文一起传入：
...
这会把 `PlanetId`、`PlanetName`、`EntityId`、`ProtoId`、`ModelIndex` 和工厂实体游标写进报告。DSPCore 只记录调用方提供的事实，不自动判断这些对象就是根因。
...
- `GetReports()` 返回当前报告快照。
- `BuildDiagnosticText(...)` 返回诊断文本；传入 `ErrorDiagnosticContext` 时会追加明确的星球/实体上下文；`CopyDiagnosticText(...)` 会同时写入系统剪贴板并返回同一份文本。
