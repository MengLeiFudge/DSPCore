...
- 错误报告包含归属模组 GUID、错误类型、消息、堆栈和可选星球/实体上下文，便于后续诊断 UI 或日志工具读取。
- Unity error / exception / assert 日志会被捕获成报告。
- `Errors.BuildDiagnosticText(...)` 会生成可复制诊断文本，包含当前游戏状态、调用方传入的星球/实体上下文、最近错误、文本命中的候选插件、DSPCore 作者声明和 Harmony patch map 概览。
...
如果错误发生时已经有明确的星球工厂或实体 ID，可以在上报异常时直接带上上下文：
...
也可以直接构造 `ErrorReport` 并调用 `Errors.Report(report)`，或使用 `Errors.Report(ownerModGuid, errorType, message, stackTrace, context)` 参数短入口。
...
诊断文本会包含当前错误文本、当前游戏状态、最近错误报告、命中文本的候选插件、已注册 feature/module/patch 声明，以及 Harmony 已 patch 方法和 owner 概览。候选插件只是基于 GUID / 名称文本命中，不能等同于根因判断。

如果只想在复制诊断文本时附加当前上下文，也可以把上下文传给诊断文本入口：
...
这会把 `PlanetId`、`PlanetName`、`EntityId`、`ProtoId`、`ModelIndex` 和工厂实体游标写进这次复制出来的诊断文本。DSPCore 只记录调用方提供的事实，不自动判断这些对象就是根因。

## 调用后 DSPCore 会怎么处理

- `Report(...)` 会把报告追加到内存列表；参数短入口会先创建 `ErrorReport`。
- `ReportException(...)` 会用异常类型、消息、堆栈和可选上下文创建 `ErrorReport` 并追加。
- `GetReports()` 返回当前报告快照。
- `BuildDiagnosticText(...)` 返回诊断文本；最近错误报告会展示报告自带的上下文；传入 `ErrorDiagnosticContext` 时还会追加本次复制的明确星球/实体上下文；`CopyDiagnosticText(...)` 会同时写入系统剪贴板并返回同一份文本。
...
- 不扫描全图寻找可疑实体、星球或工厂；实体上下文必须由调用方明确传入。
- 不自动上传日志或发送外部网络请求。
