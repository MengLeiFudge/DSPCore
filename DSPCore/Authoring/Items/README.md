...
- 常见原版字段有语义化入口：`SetIconTag(...)`、`UnlockAlways()`、`UnlockByEnemyDrop()`、`UnlockLike(...)`、`UnlockByRecipe()`、`SetPreTechOverride(...)` 和 `SetProductive(...)`。
...
`SetIconTag(...)` 写入新版 DSP 常用的 `ItemProto.IconTag`。`UnlockAlways()` 等解锁入口写入原版 `UnlockKey`，其中 `-1` 表示直接解锁、`-2` 表示黑雾掉落、正数表示跟随另一个物品解锁；也可以用 `SetUnlockKey(...)` 直接写原始值。`SetGridIndex(...)` 写入原版 `ItemProto.GridIndex`。`BindIcon(...)`、`BindEmbeddedIcon(...)` 和 `BindAssetBundleIcon(...)` 只注册图标 descriptor，图标仍由 Icons runtime 在缓存重建时应用。`RegisterItem(...)` 会把当前物品登记到 DSPCore 的 ProtoPipeline。
