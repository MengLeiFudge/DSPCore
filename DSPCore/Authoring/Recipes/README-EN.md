...
`SetGridIndex(...)` writes the native `RecipeProto.GridIndex`. `BindIcon(...)`, `BindEmbeddedIcon(...)`, and `BindAssetBundleIcon(...)` only register icon descriptors; the Icons runtime still applies them during cache rebuilds. `RegisterRecipe(...)` sends the current recipe to DSPCore's ProtoPipeline.
