...
- 用 `GridDsl.FormCard(...)`、`ListCard(...)`、`DetailCard(...)` 和 `StatusFooter(...)` 快速搭出常见工具页骨架。
...
这些 helper 只生成布局节点：表单行使用 `UiFormRowDescriptor`，列表项使用 `UiListItemDescriptor`，详情区使用 `UiDetailDescriptor`。选择项回调和具体业务状态仍由调用方持有。
...
- 不替代 `Tabs`、`PickerSurfaces`、`ErrorWindow` 等已经有明确归属的能力或系统。
