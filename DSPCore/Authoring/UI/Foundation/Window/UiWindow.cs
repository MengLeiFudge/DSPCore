...
// UiWindow modified from LSTM: https://github.com/hetima/DSP_LSTM/blob/main/LSTM/UiWindowCtl.cs
// 大部分UI来源为UXAssist，感谢@soarqin的源码以供参考！
/// <summary>
/// DSPCore 自定义窗口的基础布局、控件创建和刷新框架。
/// </summary>
public class UiWindow : ManualBehaviour {
    ...
    public UiImageButton AddImageButton(float x, float y, RectTransform parent, Proto proto = null,
        string objName = "image-button") {
        var btn = UiImageButton.CreateImageButton(x, y, parent, proto);
        btn.gameObject.name = objName;
    ...
    public UiFlatButton AddFlatButton(float x, float y, RectTransform parent, string text = "", int fontSize = 15,
        string objName = "button", UnityAction onClick = null) {
        var btn = UiFlatButton.CreateFlatButton(x, y, parent, text, Math.Max(MinFontSize, fontSize), _ => onClick());
        btn.gameObject.name = objName;
    ...
    public UiCheckBox AddCheckBox(float x, float y, RectTransform parent, ConfigEntry<bool> config, string label = "",
        int fontSize = 15) {
        var cb = UiCheckBox.CreateCheckBox(x, y, parent, config, label, Math.Max(MinFontSize, fontSize));
        _maxX = Math.Max(_maxX, x + cb.Width);
        MaxY = Math.Max(MaxY, y + cb.Height);
        return cb;
    }

    // public UiCheckBox AddCheckBox(float x, float y, RectTransform parent, bool check, string label = "",
    //     int fontSize = 15) {
    //     var cb = UiCheckBox.CreateCheckBox(x, y, parent, check, label, fontSize);
    //     _maxX = Math.Max(_maxX, x + cb.Width);
    //     MaxY = Math.Max(MaxY, y + cb.Height);
    //     return cb;
    // }

    public UiComboBox AddComboBox(float x, float y, RectTransform parent, int fontSize = 15) {
        var comboBox = UiComboBox.CreateComboBox(x, y, parent).WithFontSize(Math.Max(MinFontSize, fontSize));
        _maxX = Math.Max(_maxX, x + comboBox.Width);
        MaxY = Math.Max(MaxY, y + comboBox.Height);
        return comboBox;
    }

    public UiCornerComboBox AddCornerComboBox(float x, float y, RectTransform parent, int fontSize = 15) {
        var comboBox = UiCornerComboBox.CreateComboBox(x, y, parent).WithFontSize(Math.Max(MinFontSize, fontSize));
        _maxX = Math.Max(_maxX, x + comboBox.Width);
        MaxY = Math.Max(MaxY, y + comboBox.Height);
        return comboBox;
    }
    ...
    public UiSlider AddSlider(float x, float y, RectTransform parent, float value, float minValue, float maxValue,
        string format = "G", float width = 0f) {
        var slider = UiSlider.CreateSlider(x, y, parent, value, minValue, maxValue, format, width);
    ...
    public UiSideSlider AddSideSlider(float x, float y, RectTransform parent, float value, float minValue,
        float maxValue, string format = "G", float width = 0f, float textWidth = 0f) {
        var slider = UiSideSlider.CreateSlider(x, y, parent, value, minValue, maxValue, format, width, textWidth);
        var rect = slider.rectTrans;
        if (rect != null) {
            _maxX = Math.Max(_maxX, x + rect.sizeDelta.x);
            MaxY = Math.Max(MaxY, y + rect.sizeDelta.y);
    ...
    public UiSlider AddSlider<T>(float x, float y, RectTransform parent, ConfigEntry<T> config,
        ValueMapper<T> valueMapper, string format = "G", float width = 0f) {
        var slider = UiSlider.CreateSlider(x, y, parent, OnConfigValueChanged(config), valueMapper.Min, valueMapper.Max,
            format, width);
    ...
    public UiSideSlider AddSideSlider<T>(float x, float y, RectTransform parent, ConfigEntry<T> config,
        ValueMapper<T> valueMapper, string format = "G", float width = 0f, float textWidth = 0f) {
        var slider = UiSideSlider.CreateSlider(x, y, parent, OnConfigValueChanged(config), valueMapper.Min,
    ...
    public UiSlider AddSlider<T>(float x, float y, RectTransform parent, ConfigEntry<T> config, T[] valueList,
        string format = "G", float width = 0f) {
        return AddSlider(x, y, parent, config, new ArrayMapper<T>(valueList), format, width);
    }

    public UiSideSlider AddSideSlider<T>(float x, float y, RectTransform parent, ConfigEntry<T> config, T[] valueList,
        string format = "G", float width = 0f) {
