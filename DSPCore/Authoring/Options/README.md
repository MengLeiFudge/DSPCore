...
Options 模块让模组声明简单配置项，由 DSPCore 统一绑定到 BepInEx `ConfigFile`，并提供 DSPCore 自有统一设置窗口。公开表面包含 `String`、`Bool`、`Int`、`Float` 短入口、底层字符串注册、设置页面描述、设置版本描述和 `Options.OpenWindow()`。
...
统一设置窗口会展示所有已注册配置项：`Bool` 使用复选框，`String`、`Int`、`Float` 使用输入框，输入结束后写回 DSPCore 的 BepInEx 配置文件。窗口必须在 `UIRoot` 初始化后打开，通常从模组按钮、快捷键或自己的 UI 回调里调用。

## 边界

- 当前写入 BepInEx 的底层值仍是字符串；布尔、整数和浮点有读取辅助，枚举仍由作者解析。
- 当前窗口是 DSPCore 自有窗口，不注入原版设置页。
- `Int` / `Float` 输入非法时会回滚为当前配置值。
