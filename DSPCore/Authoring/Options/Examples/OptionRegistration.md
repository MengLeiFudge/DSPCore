...
Prefer `Options.Page(...).Section(...)` when several settings belong to one DSPCore page inside the vanilla option window. The returned `OptionSection` keeps the page id and config section, so each row only names its key, default value, and description.
...
Call `Options.OpenWindow()` only from a button, key bind, or custom UI callback after `UIRoot` is initialized. Early plugin startup is too soon for Unity UI creation. The call opens the vanilla option window and selects DSPCore's page; DSPCore does not create a separate settings window.
