...
- `Features` 和 `Modules` 能让模组声明自己的功能/模块元数据，便于其他模组查询是否存在；DSPCore 启动后再注册的新声明会立即执行一次初始化回调。
- `Patches` 能声明由 DSPCore 统一调度的条件补丁，并记录必需插件缺失、版本不足、条件禁用和应用失败；DSPCore 已持有 Harmony 后注册的新补丁会立即按同一套条件尝试应用。
...
初始化时，DSPCore 会按 priority 和 ID 顺序初始化 feature；module 当前按注册表枚举顺序初始化，不做依赖拓扑排序。DSPCore 启动后再注册的 feature/module 会立即执行一次初始化回调；同一个 ID 已初始化后再次注册只替换 descriptor，不会再次执行初始化。
...
`Patches.Register(...)` 是普通作者入口；`Patches.RegisterForPlugin(...)` 适合依赖目标插件的集成补丁。DSPCore 会在 `DSPCorePlugin.Awake()` 装配内置运行时补丁后应用已注册声明；如果其他模组在 DSPCore 启动后才注册补丁，DSPCore 会立即用已经创建的 Harmony 实例尝试应用。目标插件缺失、版本不足或 `isEnabled` 返回 false 时会记录禁用原因并跳过 `apply`。同一个 patch ID 成功应用或应用失败后不会再次执行，避免重复 patch；条件未满足的声明仍可由后续同 ID 注册替换后再次尝试。
...
- 不替模组自动解析模块依赖图；`Dependencies` 目前是描述数据。晚注册的 feature/module 仍只保证自身初始化回调执行一次，不会等待依赖补齐。
- 不承载具体功能补丁实现；`PatchRegistry` 只保存声明，具体 Harmony patch 代码仍应由所属系统或模组自己的运行时代码提供。
- 不应新增集中式旧兼容目录；旧 API shim 必须放回所属作者能力的 `Compat/`。

## 运行时启动

`DSPCorePlugin.Awake()` 会初始化 DSPCore、注册旧 DSPModSave 处理器、创建 Harmony，并装配当前已实现的 Proto、BuildBar、Saves、Achievements、Errors、Localization、Tabs、GameEnums patch，然后触发 `Lifecycle.OnStarted`。DSPCore 启动后注册的 Features、Modules 和 Patches 会走晚注册路径，不会因为依赖方 BepInEx `Awake` 顺序晚于 DSPCore 而静默失效。`Update()` 会轮询 KeyBinds、picker surface 和 `Lifecycle.OnUpdate`。存档链路事件由 `SaveRuntime` 在对应 `GameSave` / `GameData` patch 中转发。
