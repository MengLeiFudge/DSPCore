...
- 玩家可以从 DSPCore 统一设置窗口打开“建造栏绑定”编辑器，选择槽位后用原版物品选择器绑定物品、显式清空槽位或恢复作者默认绑定。
...
DSPCore 自带一个基础玩家编辑器：玩家从原版设置窗口打开“模组设置”，再点“建造栏绑定”，选择某个 `category` / `row` / `index` 槽位后，可以用 Select Item 打开原版物品选择器绑定物品，也可以 Empty 显式清空，或 Default 删除覆盖并回到作者默认绑定。
...
`OpenEditor()` 会在 `UIRoot` 初始化后打开 DSPCore 建造栏绑定编辑器，适合模组自己的按钮或快捷键复用同一套玩家编辑界面。
...
- DSPCore 统一设置窗口提供“建造栏绑定”入口；玩家在编辑器里改动槽位后，DSPCore 会立即刷新当前建造栏投射。
...
- 不接管 RebindBuildBar 的重绑 UI、快捷键或后续配置写回；DSPCore 只把已有 `CustomBarBind.cfg` 配置导入自己的玩家覆盖层。后续玩家改动由 DSPCore 自己的建造栏绑定编辑器写入 `.dspcore` 存档。
