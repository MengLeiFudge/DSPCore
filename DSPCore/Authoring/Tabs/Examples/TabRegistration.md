...
在启动阶段注册一次。普通场景使用 `Tabs.AddTab(id, ownerModGuid, title, iconId, order)` 参数重载；只有需要批量构造或保留声明对象时再使用 `CoreTabDescriptor`。当前投射覆盖原版物品选择器、配方选择器、制造器页面、信号选择器和标签图标选择器。蓝图图标、描述图标、智能输入框图标等复用原版 signal/tag picker 的界面会受益。DSPCore 不按第三方插件 GUID 跳过原版 signal/tag picker 注入；真正自建且不复用原版 picker 的第三方界面需要专门适配。
