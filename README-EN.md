...
- Resources, icons, and localization: register resource roots and translation entries through `ModResources`, and register icons through `Icons.FromResources(...)`, `Icons.FromFile(...)`, `Icons.FromEmbedded(...)`, `Icons.FromAssetBundle(...)`, or `Icons.BindToProto(...)`.
...
- `KeyBindRegistry` polls registered key bindings and invokes callbacks, including simple `Ctrl`/`Alt`/`Shift` modifier combinations. Bindings with `CanOverride=true` enter the DSPCore unified settings window, where same-key bindings in the same `ConflictGroup` show a conflict hint. Runtime prefers player config and falls back to the default key when config is empty or invalid.
...
Icons.FromAssetBundle("example-bundle", "com.example.my-mod", "assets/icons/example-icons", "example-machine");
