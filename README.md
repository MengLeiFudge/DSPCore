...
- `KeyBindRegistry` 会轮询已注册按键并调用回调；作者侧短入口是 `KeyBinds.Register(id, ownerModGuid, displayName, defaultKey, callback, ...)`，支持简单的 `Ctrl`/`Alt`/`Shift` 修饰键组合；`CanOverride=true` 的按键会注入原版 `BuiltinKey` 和 `overrideKeys` 数据模型，直接显示在原版按键页；运行时读取玩家在原版按键页保存的覆盖值，未覆盖时回落默认键。
...
`tests/verify-runtime-wiring.sh` 会检查 DSPCorePlugin 启动装配、原版设置窗口 DSPCore 分页、原版按键页接入、BuildBar 热键重绑交互、BuildBar 扩展行、Picker 动态布局、错误窗口按钮，以及实体、星球、星系、蓝图参数、模型和存档桥的关键 Harmony patch 接线。它是源码结构守卫，不替代真实游戏内截图、点击或 Nebula 双端联机验证。
