...
require "Options.OpenWindow();" "$smoke_plugin" "Smoke mod keybind must open the DSPCore vanilla option page."
require "Saves.GlobalAuto<SmokeGlobalState>" "$smoke_plugin" "Smoke mod must create GlobalSaves data."
require "Saves.SaveGlobal();" "$smoke_plugin" "Smoke mod must flush GlobalSaves without relying on a player window."
...
require "build bar binding uses vanilla build bar hotkeys" "$smoke_plugin" "Smoke mod must document that BuildBar rebinding uses vanilla build bar interaction."
...
require "ui-pages" "$evidence_script" "Evidence checker must validate the option page and non-window GlobalSaves/BuildBar logs."
