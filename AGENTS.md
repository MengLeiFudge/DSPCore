...
- P2/P3 框架块当前已包含自定义实体组件、星球/恒星/银河系统、蓝图参数块、模型/预制体克隆、配置项描述、可选联机声明、网络查询适配器和条件补丁平台。当托管类型有无参构造函数时，新示例应优先使用 `Components.Register<TComponent>(...)`、`PlanetSystems.Register<TSystem>(...)`、`GalaxySystems.RegisterGalaxy<TSystem>(...)` 等类型化短入口；普通网络适配器应优先使用 `Networks.Register(networkId, ownerModGuid, tryGetCommonNetwork, ...)`；条件补丁应优先使用 `Patches.Register(...)` 或 `Patches.RegisterForPlugin(...)`。descriptor 保留为高级构造路径。RecipeFinder、FreeMechaCustom、AssemblerUI 风格面板等玩家便利模块仍未进入核心。
